...
const BookingPage = () => {
  ...
  const handleInputChange = (field: string, value: string | number) => {
    setBookingDetails({
      ...bookingDetails,
      [field]: value,
      totalAmount: field === 'members' ? Number(value) * resortDetails.pricing : bookingDetails.totalAmount
    });
  ...
  const validateStep = (step: number) => {
    ...
    if (step === 2) {
      ...
      if (!bookingDetails.email.trim()) newErrors.email = 'Email is required';
      else if (!/\S+@\S+\.\S+/.test(bookingDetails.email)) newErrors.email = 'Email is invalid';
      if (!bookingDetails.phone.trim()) newErrors.phone = 'Phone number is required';
      else if (!/^\d{10}$/.test(bookingDetails.phone.replace(/\D/g, ''))) newErrors.phone = 'Phone number must be 10 digits';
    }
  ...
  const handleNext = () => {
    if (validateStep(currentStep)) {
      if (currentStep === 1) {
        setBookingDetails({
          ...bookingDetails,
          totalAmount: bookingDetails.members * resortDetails.pricing
        });
        setCurrentStep(2);
            ...
            {[1, 2, 3].map((step) => (
              <React.Fragment key={step}>
                ...
                {step !== 3 && (
                  <div
                    className={`w-16 h-1 ${
                      currentStep > step ? 'bg-gradient-to-r from-orange-500 to-pink-500' : 'bg-gray-200'
                    }`}
                  ></div>
                )}
              </React.Fragment>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => handleInputChange('members', parseInt(value))}
                >
                 <SelectTrigger className="w-full max-w-xs p-3 border-2 border-cyan-600 rounded-xl shadow-md focus:outline-none focus:ring-2 focus:ring-cyan-500 text-gray-700">
              ...
              <div className="bg-gradient-to-r from-orange-50 to-pink-50 p-6 rounded-lg">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between text-lg font-bold text-orange-600 pt-2 border-t">
                    ...
                    <span>₹{(bookingDetails.members * resortDetails.pricing).toLocaleString()}</span>
                  </div>
                </div>
              </div>
            </div>
